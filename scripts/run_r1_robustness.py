...
def main() -> int:
    ...
    # R01 — harness identity + bit-flip sensitivity.
    raw = (repo / "r1_scientific_harness/harness.tar.gz.b64").read_text(encoding="utf-8")
    ...
    r = receipt(
        "R01",
        observed == closure.HARNESS_TAR_SHA and sha256_bytes(corrupted) != observed,
        {"observed_sha256": observed, "expected_sha256": closure.HARNESS_TAR_SHA, "corrupted_sha256": sha256_bytes(corrupted)},
    )
    gates["R01"] = r
    write_json(out / "R01.json", r)

    # R02 — generic integrity fail-closed.
    integ = work / "integrity"
    integ.mkdir(exist_ok=True)
    p = integ / "payload.bin"
    p.write_bytes(b"ascom-00323-robustness")
    ...
    r = receipt(
        "R02",
        ok_before and not ok_after and not ok_missing,
        {"before_errors": errors_before, "corruption_errors": errors_after, "missing_errors": errors_missing},
    )
    gates["R02"] = r
    write_json(out / "R02.json", r)

    # R03 — scientific fingerprint stability/sensitivity.
    ...
    for value in (float("nan"), float("inf"), float("-inf")):
        try:
            scientific_fingerprint({"x": value})
        except ValueError:
    ...
    r = receipt(
        "R03",
        a == b and a != c and all(rejected),
        {"order_independent": a == b, "precision_sensitive": a != c, "non_finite_rejected": rejected},
    )
    gates["R03"] = r
    write_json(out / "R03.json", r)

    # Materialize the exact recovered late-time validation runtime.
    harness_work = work / "harness"
    harness_work.mkdir(parents=True, exist_ok=True)
    harness = closure.decode_harness(repo, harness_work)
    ...
    # R04 — relocate to path with spaces + Unicode.
    ...
    r = receipt(
        "R05",
        canonical_sha == "9ac154ab583ce759c0f7eef3c978c7c70a6ead2d18774caceadf1a350a640585" and sha256_bytes(crlf) != canonical_sha,
        {"canonical_sha256": canonical_sha, "crlf_sha256": sha256_bytes(crlf), "canonical_authority": "Git blob"},
    )
    gates["R05"] = r
    write_json(out / "R05.json", r)

    # R06 — ACT payload bit-flip detection.
    act = repo / ".r1/act_dr6_cmbonly.bin"
    if not act.is_file():
        raise RuntimeError(f"ACT payload missing from core materialization: {act}")
    act_bytes = act.read_bytes()
    bitflip = bytearray(act_bytes)
    bitflip[len(bitflip) // 2] ^= 1
    r = receipt(
        "R06",
        sha256_bytes(act_bytes) == ACT_SHA and sha256_bytes(bitflip) != ACT_SHA,
        {"source_sha256": sha256_bytes(act_bytes), "expected_sha256": ACT_SHA, "bitflip_sha256": sha256_bytes(bitflip), "bytes": len(act_bytes)},
    )
    gates["R06"] = r
    write_json(out / "R06.json", r)

    # R07 — five fresh CAMB processes.
    ...
    gates["R07"] = r
    write_json(out / "R07.json", r)

    # R08 — thread-count perturbation.
    ...
    gates["R08"] = r
    write_json(out / "R08.json", r)

    # R09 — cold/warm/post-clear cache identity.
    adapter = TieredCobayaAdapterV5(campaign, quadrature_order=64, shape_cache_capacity=4, scale_cache_capacity=4)
    cold = adapter.logp(**nominal)
    m1 = adapter.metrics()
    warm = adapter.logp(**nominal)
    m2 = adapter.metrics()
    adapter.clear()
    cleared = adapter.logp(**nominal)
    m3 = adapter.metrics()
    pass_r09 = (
        max(abs(cold - warm), abs(cold - cleared)) <= TOL
        and m2["shape_hits"] > m1["shape_hits"]
        and m2["scale_hits"] > m1["scale_hits"]
        and m3["shape_computations"] > m2["shape_computations"]
    )
    ...
    gates["R09"] = r
    write_json(out / "R09.json", r)

    # R10 — four independent 64-point LHS seeds.
    seed_rows = []
    for seed in SEEDS:
    ...
    gates["R10"] = r
    write_json(out / "R10.json", r)

    # R11 — all 32 corners of frozen five-dimensional domain.
    names = ("omega_m", "H0", "sigma8", "rdrag_mpc", "M_B")
    corners = [dict(zip(names, values, strict=True)) for values in itertools.product(*(PARAMETER_BOUNDS[n] for n in names))]
    ...
    gates["R11"] = r
    write_json(out / "R11.json", r)

    # R12 — force LRU eviction then replay an evicted point.
    ...
    for point in points:
        tiny.logp(**point)
    before = tiny.metrics()
    replay = tiny.logp(**points[0])
    baseline_replay = _baseline_logp(campaign, points[0], quadrature_order=64)
    after = tiny.metrics()
    r = receipt(
        "R12",
        abs(replay - baseline_replay) <= TOL and before["shape_evictions"] > 0 and before["scale_evictions"] > 0,
        {"shape_evictions": before["shape_evictions"], "scale_evictions": before["scale_evictions"], "replay_abs_delta": abs(replay - baseline_replay), "metrics_after_replay": after},
    )
    gates["R12"] = r
    write_json(out / "R12.json", r)

    # R13 — negative controls inherited from paper validation.
    negative = run_failure_injection(runtime)
    r = receipt("R13", negative.get("status") == "pass", negative)
    gates["R13"] = r
    write_json(out / "R13.json", r)

    # R14 — 4x memory-duration stress with same 128 MiB ceiling.
    ...
    gates["R14"] = r
    write_json(out / "R14.json", r)

    # R15 — deterministic Pantheon+ compilation across independent roots.
    rta = work / "pantheon-rebuild-a"
    rtb = work / "pantheon-rebuild-b"
    a_pan = closure.compile_pantheon(rta, pan)
    b_pan = closure.compile_pantheon(rtb, pan)
    ...
    gates["R15"] = r
    write_json(out / "R15.json", r)

    # R16 — fresh campaign object with same seed reproduces numerical decision.
    ...
    r = receipt(
        "R16",
        first["status"] == "pass" and second["status"] == "pass" and first["max_abs_log_likelihood_delta"] == second["max_abs_log_likelihood_delta"],
        {"first_max_delta": first["max_abs_log_likelihood_delta"], "second_max_delta": second["max_abs_log_likelihood_delta"], "seed": 20260803},
    )
    gates["R16"] = r
    write_json(out / "R16.json", r)
