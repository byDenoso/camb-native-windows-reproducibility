...
def decode_canonical_harness(repo: Path, work: Path) -> tuple[Path, bytes]:
    ...
    if sha256_bytes(manifest_bytes) != MANIFEST_SHA256:
        raise RuntimeError('scientific harness manifest identity mismatch')
    ...
    if actual_paths != expected_paths:
        ...
        raise RuntimeError(f'scientific harness file-set mismatch: extra={extra}, missing={missing}')

    if not (root / 'peer_platform/paper_validation_v6.py').is_file():
        raise RuntimeError('scientific harness extraction incomplete')
    return root, manifest_bytes


legacy.HARNESS_SHA = MANIFEST_SHA256
