...
def main() -> int:
    ...
    passed = objective_delta <= TOL and chi2_delta <= TOL and model_delta <= 1e-6
    payload = {
        'gate_id': 'G07', 'status': 'verified' if passed else 'failed',
        'schema': 'ascom-00323-g07-act-kernel-parity/v2',
        ...
        'objective_absolute_delta': float(objective_delta), 'chi2_absolute_delta': float(chi2_delta),
        ...
        'claim_boundary': 'Fresh test certifies direct/projected ACT nuisance-kernel numerical parity on official payload bytes. Historical K-PEER timing is lineage only unless reproduced by this run.',
    }
